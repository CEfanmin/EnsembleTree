...
class MGCForest():
    ...
    def fit(self, X, y):
        scanned_X = np.hstack([mgs.fit(X, y) for mgs in self.mgs_instances])
...
class MultiGrainedScanner():
    ...
    @param verbose              Adds verbosity.
    """
    def __init__(self, estimators_config, stride_ratio=0.25, folds=3, verbose=False):
    ...
    def predict(self, X):
        ...
        sliced_X = self.slices(X)
        return np.hstack([
            estimator.predict_proba(window_X)
